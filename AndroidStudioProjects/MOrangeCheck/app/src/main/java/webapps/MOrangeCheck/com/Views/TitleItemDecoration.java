...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    private List<MonthTimeBean> mDatas;
    ...
    public TitleItemDecoration(Context context, List<MonthTimeBean> datas) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
...
/*
        ...
        c.drawText(mDatas.get(position).getTag(), child.getPaddingLeft()+(mTitleHeight/2), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        c.drawText(tag, child.getPaddingLeft()+(mTitleHeight/2),
