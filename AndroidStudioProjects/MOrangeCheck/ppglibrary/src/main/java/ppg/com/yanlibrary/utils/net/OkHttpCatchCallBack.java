...
/**
 * 缓存模式的回调,onBefore判断是否有缓存,有的话直接在那里返回数据
 * Created by ppg on 2016/4/14.
 */
public abstract class OkHttpCatchCallBack {
    /**
    ...
    public static abstract class PostCatchTaskCallBack extends Callback<JsonBaseBean> {
        ...
        public void onBefore(Request request, int id) {
            ...
            if (JsonFileCache.isExistData(mContent, url)) {
                String content = JsonFileCache.extractData(mContent, url);
                JsonBaseBean object = new JsonBaseBean();
                object.analysisJson(content);
                onResponse(object, id);
                onAfter(id);
            }
            //开启进度条
            if (mContent != null) {
